...
### [#](#_1-用户未授权设备-errCode-9) (1) 用户未授权设备 (errCode: 9)
...
- 用户授权了设备组 A，但设备被使用 [`addIotGroupDevice`](https://developers.weixin.qq.com/miniprogram/dev/server/API/hardware-device/api_addiotgroupdevice) (force\_add=true) 强制转移到了另一设备组 B，也会导致设备从设备组 A 里移除。
...
### [#](#_2-设备呼叫手机微信-voipToken-错误-errCode-13) (2) 设备呼叫手机微信 voipToken 错误 (errCode: 13)
...
## [#](#_2-「拨打方」加入阶段) 2. 「拨打方」加入阶段

创建 VoIP 房间成功后，「拨打方」会直接加入该房间，界面上会显示「连接中...」。加入的时长一般与当前网络状态有关。

加入成功后，拨打方会触发 `joinedRoomByCaller` 事件。

### [#](#_1-设备之前可以拨打成功，突然开始持续失败，需重启-WMPF-才能恢复) (1) 设备之前可以拨打成功，突然开始持续失败，需重启 WMPF 才能恢复
...
### [#](#_2-一直显示「连接中」（卡在本阶段），无法加入房间，通话被突然结束) (2) 一直显示「连接中」（卡在本阶段），无法加入房间，通话被突然结束

可能是网络状况较差导致加入过慢，此时通话可能会因接听方等待超时而结束，触发 `abortVoip` 和 `endVoip` 事件。

### [#](#_3-未加入房间就直接退出通话) (3) 未加入房间就直接退出通话
...
## [#](#_4-「接听方」加入（接听）阶段) 4. 「接听方」加入（接听）阶段
...
### [#](#_1-点接听时没有接通，通话反而直接结束) (1) 点接听时没有接通，通话反而直接结束
...
### [#](#_2-用户还没选择接听或挂断，通话就结束了，提示通话被拒绝) (2) 用户还没选择接听或挂断，通话就结束了，提示通话被拒绝

（VoIP 插件 < 2.3.2 版本）用户进行锁屏、退出微信或退出小程序，也会被视为拒接，会收到 `rejectVoip` 事件和 `endVoip` 事件；

## [#](#_5-「接听方」等待阶段) 5. 「接听方」等待阶段
...
### [#](#_1-接听后一直未开始通话，或等待一会后通话自动结束) (1) 接听后一直未开始通话，或等待一会后通话自动结束
...
### [#](#_1-用户没点击挂断，通话自己结束) (1) 用户没点击挂断，通话自己结束
