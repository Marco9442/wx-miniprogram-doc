# [#](#小程序音视频通话-SDK-Linux-设备) 小程序音视频通话 SDK (Linux 设备)
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| config | wx\_init\_config\_t\* | 初始化参数，后面详细说明 |
| get\_module | wx\_get\_module\_t | [硬件抽象层](../voip-sdk/hal)配置的函数指针 |

**返回值**

[`wx_operation_t`](../voip-sdk/wx_operation) wx\_init()是异步函数，会发起网络请求。请留意异步操作完成之后的状态码。

| 错误码 | 说明 |
| --- | --- |
| WXERROR\_INVALID\_ARGUMENT | config 中的参数有错误。 |
| WXERROR\_TIMEOUT | 请求超时。 |
| WXERROR\_RESOURCE\_EXHAUSTED | 请检查网络状态或磁盘读写权限。 |
| WXERROR\_FAILED\_PRECONDITION | config 传入的 device\_id 和 device\_signature 没有在 WeCooper 平台注册；或重复调用 wx\_init()。 |
| WXERROR\_INTERNAL | 其他内部错误。 |

### [#](#_4-1-wx-init-config-t) 4.1 `wx_init_config_t`

提供初始化 SDK 所需的一系列参数。

| 属性 | 类型 | 说明 |
| --- | --- | --- |
| common | wx\_struct\_t | wx\_init\_config\_t 类型的基类 |
| log\_dir | char \* | SDK 写日志的路径。如果设置为`NULL`则不会写入日志 |
| data\_dir | char \* | SDK 读写数据的路径。需确保进程有该目录的读写权限，需确保此目录内容不被其他代码/进程修改。 |
| product\_id | int | 固定填 `0` |
| host\_appid | char \* | 固定填 `NULL` |
| device\_id | char \* | 厂商自行分配的设备 ID。需确保每台设备使用唯一的 ID，并且同一设备始终使用同一 ID。 |
| device\_signature | char \* | 固定填 `NULL` |
| device\_signature\_version | int | 固定填 `0` |
| model\_id | char \* | 通过[硬件设备接入](../device-access)获得的设备型号 ID |
| wxa\_appid | char \* | 接收通话的小程序的 AppID |
| wxa\_flavor | wx\_wxa\_flavor\_t | 接收通话的小程序的版本，如开发版、体验版、正式版。 |
| rpmb\_device | char \* | RPMB 设备路径。采用 EMMC 的硬件设备需要指定此路径。 |
| h265\_only | bool | 如果设备只支持 H265，可以设置此字段为 true，否则设备接收到的流以 H264 优先 |
| video\_landscape | bool | true: SDK 收到流的宽高比 4:3, false: SDK 收到流的宽高比 3:4 |
| subscribe\_video\_length | int | 开发者可以用此配置来订阅一个分辨率的长边值，目前支持 320，需要向微信提交 appid，待开通订阅机制后生效 |
...
### [#](#_5-1-检查当前设备是否已注册) 5.1 检查当前设备是否已注册
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| is\_registered\_out | bool\* | 传入指针，用于接收是否注册的状态，如果检查到尚未注册，需要参考流程 5.2 进行设备注册 |
...
| --- | --- |
| WXERROR\_OK | 成功 |
| WXERROR\_FAILED\_PRECONDITION | wx\_init()函数尚未调用。需要先调用 wx\_init()。 |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| sn\_ticket | char\* | 通过[获取设备票据](https://developers.weixin.qq.com/miniprogram/dev/server/API/hardware-device/api_getsnticket)接口获得 |

**返回值**

[`wx_operation_t`](../voip-sdk/wx_operation) wx\_device\_register()是异步函数，会发起网络请求。请留意异步操作完成之后的状态码。

| 错误码 | 说明 |
| --- | --- |
| WXERROR\_OK | 成功 |
| WXERROR\_INVALID\_ARGUMENT | sn\_ticket 不合法或已失效。 |
| WXERROR\_ALREADY\_EXISTS | 设备已经注册过，无需二次注册。 |
| WXERROR\_PERMISSION\_DENIED | 该设备 ID 已经注册过。 |
| WXERROR\_UNAVAILABLE | 网络访问失败。 |
| WXERROR\_DATA\_LOSS | 写入注册信息失败。需要检查文件读写权限。如果是 EMMC，需检查 RPMB 是否已经写过内容。 |
| WXERROR\_FAILED\_PRECONDITION | wx\_init()函数尚未调用。需要先调用 wx\_init()。 |

## [#](#_6-配置-Session) 6. 配置 Session

> 头文件：`wmpf/voip.h`。

发起通话之前，需要首先创建一个 Session。Session 中将配置通话发起方（也就是接入此 SDK 的设备）的信息，设置通话状态回调等。

### [#](#_6-1-初始化-Session-对象) 6.1 初始化 Session 对象
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| session\_type | wx\_voip\_session\_type\_t | 选择是纯音频通话还是音视频通话 |
| scene | wx\_voip\_session\_scene\_t | 固定填 `WX_VOIP_SESSION_SCENE_IOT` |
| listener | wx\_voip\_session\_listener \* | 设置监听通话状态回调 |
| user\_data | void \* | 开发者按需设置 context 信息。相关内容可以在状态回调中获取到。 |
| self\_member | wx\_voip\_member\_t \* | 通话发起方（也就是接入此 SDK 的设备）的信息，详见后续「self\_member 参数说明」 |
| config | wx\_voip\_session\_config\_t \* | 摄像头配置（分辨率、旋转等）。纯音频通话填入默认值{0,0,0}即可。 |
| session | wx\_voip\_session\_t \* | 本次配置的 session 对象。**注意**：后续需记得调用 wx\_voip\_session\_destroy()销毁此对象。 |
...
| --- | --- |
| WXERROR\_OK | 成功 |
| WXERROR\_INVALID\_ARGUMENT | 参数错误。 |
| WXERROR\_FAILED\_PRECONDITION | wx\_init()函数尚未调用。需要先调用 wx\_init()。 |
...
| --- | --- | --- |
| name | char \* | 固定填空字符串 |
| id | char \* | 必须传入调用 wx\_init()时 config 中的 device\_id |
| camera\_status | wx\_voip\_member\_camera\_status\_t | 音视频通话，此参数描述通话建立时己方摄像头是否开启。纯音频通话忽略。 |

### [#](#_6-2-销毁-Session-对象) 6.2 销毁 Session 对象
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| session | wx\_voip\_session\_t \* | 要销毁的 session 对象。 |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| session | wx\_voip\_session\_t | 前面创建的 session 对象 |
| callee | const wx\_voip\_member\_t\* | 通话接听端（小程序端）信息。详见后续「callee 参数说明」 |

**返回值**

[`wx_operation_t`](../voip-sdk/wx_operation) wx\_voip\_session\_call()是异步函数，会发起网络请求。此异步函数返回成功，表示通话正确拨出，不代表通话已接通。请留意异步操作完成之后的状态码。

| 错误码 | 说明 |
| --- | --- |
| WXERROR\_OK | 成功 |
| WXERROR\_FAILED\_PRECONDITION | 当前正在通话中，不要重复拨打电话。 |
| WXERROR\_RESOURCE\_EXHAUSTED | 请检查网络状态。 |
| WXERROR\_PERMISSION\_DENIED | 被拨打的用户未向当前设备授权拨打 VoIP 通话权限；或当前小程序开通拨打 VoIP 通话的权限。 |
| WXERROR\_INVALID\_ARGUMENT | open\_id 或 sn\_ticket 设置错误 |
| WXERROR\_UNAVAILABLE | 当前微信小程序没有开通 VoIP 通话权限，或通话时长不足，或者当前设备没有购买 LICENSE。 |
| WXERROR\_INTERNAL | 内部错误 |
...
| id | char \* | 传入通话接收方微信用户在小程序里的 open\_id。 |
| camera\_status | wx\_voip\_member\_camera\_status\_t | 音视频通话，此参数描述通话建立时手机微信小程序端摄像头是否开启。纯音频通话请忽略此参数。 |

### [#](#_7-2-主动结束通话) 7.2 主动结束通话
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| session | wx\_voip\_session\_t | `wx_voip_session_new` 创建的 session 对象 |
| reason | wx\_voip\_hangup\_reason\_t | 挂断 VoIP 通话的原因 |

**返回值**

[`wx_operation_t`](../voip-sdk/wx_operation) wx\_voip\_session\_hangup()是异步函数，会发起网络请求。此异步函数返回成功，表示通话成功挂断。请留意异步操作完成之后的状态码。

### [#](#_7-3-手机微信呼叫设备-接听通话) 7.3 手机微信呼叫设备 - 接听通话
...
[`wx_operation_t`](../voip-sdk/wx_operation) wx\_voip\_listener\_join()是异步函数，会发起网络请求。此异步函数返回成功，表示通话正常加入。请留意异步操作完成之后的状态码。

### [#](#_7-4-手机微信呼叫设备-提前挂断) 7.4 手机微信呼叫设备 - 提前挂断
...
**说明**
此接口不需要 session，在 wx\_init 后即可直接调用。
若已创建了 session，则需要使用 wx\_voip\_session\_hangup 来进行挂断。

**返回值**

[`wx_operation_t`](../voip-sdk/wx_operation) wx\_voip\_listener\_hangup()是异步函数，会发起网络请求。此异步函数返回成功，表示通话成功挂断。请留意异步操作完成之后的状态码。

### [#](#_7-5-获取通话房间号-roomId) 7.5 获取通话房间号 roomId
...
| 错误码 | 说明 |
| --- | --- |
| WXERROR\_OK | SDK 正常关闭 |

## [#](#_9-常见问题) 9. 常见问题

### [#](#_9-1-SDK-链接失败) 9.1 SDK 链接失败
...
### [#](#_9-2-device-key-get-fail-10008) 9.2 device/key/get fail: 10008

这说明你在调用 `wx_device_register` 函数时传入的 SNTicket 不正确或者已经过期。

### [#](#_9-3-device-call-fail-9) 9.3 device/call fail: 9
...
### [#](#_9-4-device-call-fail-12) 9.4 device/call fail: 12

如果 SDK 日志中报该错误，有两种情况：

#### [#](#小程序未开通音视频权限) 小程序未开通音视频权限
...
### [#](#_9-5-device-call-fail-13) 9.5 device/call fail: 13
...
### [#](#_9-6-device-call-fail-20) 9.6 device/call fail: 20

传入的 OpenID 不正确，不是合法的 OpenID。请确保传入的 OpenID 由小程序 wx.login 获得。

### [#](#_9-7-device-register-HTTP-0-XX) 9.7 device/register: HTTP 0 XX
...
### [#](#_9-8-device-register-errcode-9800011-errmsg-sn-ticke-不合法) 9.8 device/register: errcode: 9800011 errmsg: sn\_ticke 不合法
...
### [#](#_9-9-device-register-errcode-9-errmsg-device-xxx-not-registered) 9.9 device/register: errcode: 9 errmsg: device `xxx` not registered
...
### [#](#_9-10-CheckMemoryLeak-Detected-memory-leak-wx-operation-leak-objects-1) 9.10 [CheckMemoryLeak]: Detected memory leak: wx\_operation, leak objects: 1
...
### [#](#_9-11-Found-local-timestamp-incorrect-expected-XXX-actual-XXX) 9.11 Found local timestamp incorrect, expected XXX, actual XXX

这个错误说明你的系统的时间戳不正确，请确保设备系统的时间戳和当前时间差距不要超过 5 秒钟.

SDK 通过 `time(NULL)` 函数获得系统时间戳, 由于时间戳是不带时区的, 请确保通过这种方式获得时间戳和 UTC 时间匹配.

### [#](#_9-12-小程序内显示设备端采集的摄像头图像被旋转了-90-度-画面拉伸变形) 9.12 小程序内显示设备端采集的摄像头图像被旋转了 90 度/画面拉伸变形
...
### [#](#_9-13-wx-operation-validate-invalid-address-wx-operation-0xXXXXXXXXX) 9.13 [wx\_operation\_validate]: invalid address: wx\_operation 0xXXXXXXXXX

说明你在调用 `wx_operation_wait`、`wx_operation_await`、`wx_operation_destroy` 等函数时传入的 `wx_operation` 对象地址不正确，或者对象已经被释放. 请检查代码的正确性。

### [#](#_9-14-Call-wx-camera-stream-listener-data-with-stream-0xXXXXXXXX-user-data-0xXXXXXXXX-But-it-s-released) 9.14 Call wx\_camera\_stream\_listener::data with stream 0xXXXXXXXX, user\_data 0xXXXXXXXX. But it's released.
