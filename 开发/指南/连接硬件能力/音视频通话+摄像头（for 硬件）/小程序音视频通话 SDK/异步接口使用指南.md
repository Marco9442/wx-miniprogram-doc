...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| operation | wx\_operation\_t | 想要等待的 operation 对象 |
| timeout\_ms | uint32\_t | 超时时间，单位毫秒。设置为 `0` 表示不超时。 |
...
| 错误码 | 说明 |
| --- | --- |
| WXERROR\_OK | 接口调用正常完成，operation 对象会被自动释放。 |
| WXERROR\_TIMEOUT | operation 超时。 |
...
| --- | --- | --- |
| operation | wx\_operation\_t | 想要等待的 operation 对象 |
| callback | wx\_operation\_callback\_t | 回调函数 |
| user\_data | void\* | 想要在回调函数里接收的自定义数据。 |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| operation | wx\_operation\_t | 想要取消的 operation 对象 |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| operation | wx\_operation\_t | 想要释放的 operation 对象 |
