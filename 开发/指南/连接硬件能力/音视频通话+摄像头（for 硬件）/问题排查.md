...
#### [#](#通话相关异常，请参考《通话异常排查指南》) 通话相关异常，请参考[《通话异常排查指南》](guide)

## [#](#_1-功能相关（通用）) 1. 功能相关（通用）

##### [#](#_1-1-如何限制用户的单次通话时长？) 1.1 如何限制用户的单次通话时长？
...
##### [#](#_1-2-在门禁、门锁场景，如何在手机端通话页面实现「开门」等功能？) 1.2 在门禁、门锁场景，如何在手机端通话页面实现「开门」等功能？
...
##### [#](#_1-3-用户如何取消授权？) 1.3 用户如何取消授权？

用户可以在小程序设置页里取消授权，或通过在最近使用中删除小程序来清空授权记录。请参考「[处理授权失效的情况](auth#_2-处理授权失效的情况)」。

##### [#](#_1-4-如何查询用户是否已授权设备（组）？) 1.4 如何查询用户是否已授权设备（组）？

请参考「[授权状态查询](auth#_4-授权状态查询)」。

##### [#](#_1-5-如何设置呼叫超时时长（长时间不接听时停止呼叫）？是否支持轮询呼叫？) 1.5 如何设置呼叫超时时长（长时间不接听时停止呼叫）？是否支持轮询呼叫？
...
##### [#](#_1-6-如何自定义手机端看到的设备端来电名称？) 1.6 如何自定义手机端看到的设备端来电名称？
...
##### [#](#_1-7-使用物联网卡时，如何配置域名和-IP-白名单？) 1.7 使用物联网卡时，如何配置域名和 IP 白名单？
...
##### [#](#_1-8-音视频通话的流量使用情况？) 1.8 音视频通话的流量使用情况？

根据测算，语音通话大概是 2MB/分钟，视频是 10-30MB/分钟。

##### [#](#_1-9-设备无摄像头或因隐私等原因不希望传画面（门禁、门锁的用户端），如何默认禁用摄像头？) 1.9 设备无摄像头或因隐私等原因不希望传画面（门禁、门锁的用户端），如何默认禁用摄像头？
...
##### [#](#_1-10-为何推送消息显示的通话时长和通话结束或者-endVoip-事件获取的不一致？应该如何获取准确的通话时长？) 1.10 为何推送消息显示的通话时长和通话结束或者 endVoip 事件获取的不一致？应该如何获取准确的通话时长？
...
## [#](#_2-功能相关（安卓设备）) 2. 功能相关（安卓设备）

##### [#](#_2-1-安卓应用和小程序之间如何进行参数传递和通信？安卓应用如何接收小程序发来的消息？) 2.1 安卓应用和小程序之间如何进行参数传递和通信？安卓应用如何接收小程序发来的消息？
...
##### [#](#_2-2-通话完成后如何关闭小程序？) 2.2 通话完成后如何关闭小程序？
...
##### [#](#_2-3-如何判断当前小程序是在设备端（WMPF）还是手机端打开) 2.3 如何判断当前小程序是在设备端（WMPF）还是手机端打开
...
## [#](#_3-异常相关（通用）) 3. 异常相关（通用）

##### [#](#_3-1-手机端未收到微信通话强提醒或提醒强度不符合预期（锁屏未提醒、未响铃、未震动等）) 3.1 手机端未收到微信通话强提醒或提醒强度不符合预期（锁屏未提醒、未响铃、未震动等）

请参考[《通话提醒异常排查指南》](notification)。

##### [#](#_3-2-获取设备票据-getSnTicket-接口返回-48001-api-unauthorized) 3.2 获取设备票据 getSnTicket 接口返回 48001 (api unauthorized)
...
##### [#](#_3-3-wx-requestDeviceVoIP-报错-invalid-scope) 3.3 `wx.requestDeviceVoIP` 报错 `invalid scope`

小程序 appId 未完成[硬件设备接入](../device-access)或接入后未申请「小程序音视频能力」设备能力导致。请确认小程序已在「小程序管理后台」完成硬件设备接入并申请通过小程序音视频能力。

##### [#](#_3-4-wx-getEnterOptionsSync-或插件的-getPluginEnteroptions-无法获取到进入小程序的-query) 3.4 `wx.getEnterOptionsSync` 或插件的 `getPluginEnteroptions` 无法获取到进入小程序的 query
...
##### [#](#_3-5-接听方接听时提示「页面不存在」) 3.5 接听方接听时提示「页面不存在」
...
##### [#](#_3-6-发起通话后，插件页面一直停留在「等待进行通话」界面无反应) 3.6 发起通话后，插件页面一直停留在「等待进行通话」界面无反应
...
##### [#](#_3-7-为什么我在-wecopper-的设备管理里找不到公钥？) 3.7 为什么我在 wecopper 的设备管理里找不到公钥？

这是因为你的设备类型是微信支付刷脸设备，目前这类设备不支持硬件 Voip 模式，需要重新申请设备类型。

## [#](#_4-异常相关（安卓设备）) 4. 异常相关（安卓设备）

##### [#](#_4-1-WMPF-获取不到正确的摄像头，或摄像头画面旋转) 4.1 WMPF 获取不到正确的摄像头，或摄像头画面旋转
...
##### [#](#_4-2-设备端有时收不到推送的通话提醒) 4.2 设备端有时收不到推送的通话提醒
...
##### [#](#_4-3-设备端为什么会需要用户微信扫码登录？) 4.3 设备端为什么会需要用户微信扫码登录？
...
##### [#](#_4-4-WMPF-ActivateDevice-报错-signature-err-1) 4.4 WMPF ActivateDevice 报错 `signature err 1`
...
- WMPF 文档中提供的[辅助工具](https://developers.weixin.qq.com/doc/oplatform/Miniprogram_Frame/utils)只适用于调试，mmIoT\_ecdsa\_sign.sh 脚本或 java 接口生成签名时，不支持并发操作。

##### [#](#_4-5-WMPF-调用接口时回调-onBridgeNotFound，或提示-bridge-not-found) 4.5 WMPF 调用接口时回调 onBridgeNotFound，或提示 `bridge not found`
...
##### [#](#_4-6-WMPF-调用时报错-Failed-to-find-provider-info-for-com-tencent-wmpf-comm-provider) 4.6 WMPF 调用时报错 `Failed to find provider info for com.tencent.wmpf.comm.provider`
...
##### [#](#_4-7-WMPF-ActivateDevice-报错-10013-request-timeout) 4.7 WMPF ActivateDevice 报错 `10013 request timeout`
...
##### [#](#_4-8-WMPF-调用-v2-版接口时报错-call-WMPF-apk-fail-is-it-installed-errCode-10001) 4.8 WMPF 调用 v2 版接口时报错 `call WMPF.apk fail, is it installed? (errCode=10001)`
...
##### [#](#_4-9-WMPF-报错-err-TRANSFER-errCode-33) 4.9 WMPF 报错 `err:TRANSFER errCode:33`
...
##### [#](#_4-10-小程序运行时报错-wmpf-is-undefined) 4.10 小程序运行时报错 `wmpf is undefined`
...
##### [#](#_4-11-在-WMPF-里使用错误的-SN-等信息激活失败后，修改为正确的-SN-仍异常。) 4.11 在 WMPF 里使用错误的 SN 等信息激活失败后，修改为正确的 SN 仍异常。
