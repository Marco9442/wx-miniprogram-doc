# [#](#小程序聊天工具开放模式开发指南) 小程序聊天工具开放模式开发指南
...
#### [#](#_1-1-wx-getGroupEnterInfo) 1.1 wx.getGroupEnterInfo
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| allowSingleChat | boolean | 为 true 时才会返回单聊下的 open\_single\_roomid |
| needGroupOpenID | boolean | 为 true 时才会返回 group\_openid |
...
| open\_single\_roomid | string | 单聊唯一标识，绑定的聊天室为单聊时返回 |
| group\_openid | string | 当前微信用户在此聊天室下的唯一标识，同一个用户在不同的聊天室下id不同 |
| chat\_type | number | 微信聊天类型，1：微信联系人单聊；2：企业微信联系人单聊；3：普通微信群聊；4：企业微信互通群聊 |

#### [#](#_1-2-wx-getChatToolInfo) 1.2 wx.getChatToolInfo
...
#### [#](#_1-3-wx-selectGroupMembers) 1.3 wx.selectGroupMembers
...
#### [#](#_1-4-open-data-list-open-data-item) 1.4 `<open-data-list>` `<open-data-item>`
...
### [#](#_2-发送到聊天能力) 2. 发送到聊天能力

可支持小程序卡片、提醒消息、文本、图片、表情、文件的发送，如果选择了多个群聊，单次仅支持将内容发送到特定的单个群聊中。

#### [#](#_2-1-wx-shareAppMessageToGroup) 2.1 wx.shareAppMessageToGroup
...
#### [#](#_2-2-wx-notifyGroupMembers) 2.2 wx.notifyGroupMembers
...
#### [#](#_2-4-wx-shareImageToGroup) 2.4 wx.shareImageToGroup
...
#### [#](#_2-5-wx-shareEmojiToGroup) 2.5 wx.shareEmojiToGroup
...
#### [#](#_2-6-wx-shareVideoToGroup) 2.6 wx.shareVideoToGroup
...
#### [#](#_2-7-wx-shareFileToGroup) 2.7 wx.shareFileToGroup
...
1.  服务端通过[创建activity\_id](https://developers.weixin.qq.com/miniprogram/dev/server/API/mp-message-management/updatable-message/api_createactivityid)接口创建 activity\_id
...
| --- | --- | --- |
| 动态消息发布者在小程序卡片中看到的辅标题 | target\_state=1或2：X人已完成  target\_state=3：已结束 | target\_state=1或2：X人已参与  target\_state=3：已结束 |
| 参与者在小程序卡片中看到的辅标题 | participator\_state=0时：  ● target\_state=1：未完成  ● target\_state=2：即将截止  ● target\_state=3：已结束  participator\_state=1时：  ● target\_state=1或2：已完成  ● target\_state=3：已结束 | participator\_state=0时：  ● target\_state=1：未参与  ● target\_state=2：即将截止  ● target\_state=3：已结束  participator\_state=1时：  ● target\_state=1或2：已参与  ● target\_state=3：已结束 |
| 非参与者在小程序卡片中看到的辅标题 | target\_state=1或2：你无需完成  target\_state=3：已结束 | target\_state=1或2：你无需参与  target\_state=3：已结束 |
...
| activity\_id | string | 是 | 动态消息的 activityId，通过步骤1中的接口获取 |
| target\_state | number | 是 | 活动状态，初始值为1（表示开始收集态），此处可设置为2（即将截止态）、3（已结束态） |
| participator\_info\_list | array | 否 | 更新后的聊天室成员状态，当target\_state=1时必填，target\_state=2或3时无需填写 |
| version\_type | number | 是 | 系统消息文字链打开的小程序版本，0 正式版，1 开发版，2 体验版 |

participator\_info\_list参数如下：

| 属性 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| group\_openid | string | 是 | 聊天室成员在此聊天室下的唯一标识 |
...
### [#](#_4-聊天工具模式内禁用的能力) 4. 聊天工具模式内禁用的能力
