...
微信聊天内素材的打开方式增加使用小程序打开的入口。用户可通过小程序处理聊天内的文件、图片、视频和 webview（包含公众号文章）。例如用小程序将文件存储到网盘、给图片加滤镜、进行视频剪辑或者将 webview/公众号文章保存到笔记等。目前仅支持不带二维码的图片直接通过小程序打开。
![](https://res.wx.qq.com/op_res/1NAvnoDPWLaLdhQPue14gmSS8Ig8UmvfxddbTVMzB-Ms8wQHubiWDHx6mJGiXFh5Vfrpy-MKkFmbYeBv3j4Byg)
