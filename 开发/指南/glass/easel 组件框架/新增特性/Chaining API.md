...
## [#](#Chaining-API-接口形式) Chaining API 接口形式
...
使用 Chaining API 的主要好处是它具有更好的 TypeScript 支持，且对于复杂组件更加友好，还可以配合 [init 函数](chaining-api-init) 来使用。但它也使得对简单组件的定义看起来稍显繁琐。
...
以下链式调用项也是可用的，但通过 [init 函数](chaining-api-init) 来调用通常更加友好。
...
## [#](#在链式调用项中使用-behavior) 在链式调用项中使用 behavior
