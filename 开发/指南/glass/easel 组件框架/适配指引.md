...
**[*Skyline* 渲染引擎](../../runtime/skyline/introduction)** 目前全量运行在 *glass-easel* 上，因此页面 **必须** 适配 *glass-easel* 才能正常运行，在开发者工具中预览或上传时会有相应的提示和检查；
...
![AppRouteLog](https://res8.wxqcloud.qq.com.cn/wxdoc/5964033a-7021-485a-af6e-cbdf8c190261.png)
...
6. [必须] [仅 Skyline] Skyline 渲染后端上的 Worklet 回调函数名称变更
   ...
   - 变更对应：
     ...
     | --- | --- | --- |
     | [pan-gesture-handler](../../runtime/skyline/gesture#通用属性) | `on-gesture-event` | `worklet:ongesture` |
     | [pan-gesture-handler](../../runtime/skyline/gesture#通用属性) | `should-response-on-move` | `worklet:should-response-on-move` |
     | [pan-gesture-handler](../../runtime/skyline/gesture#通用属性) | `should-accept-gesture` | `worklet:should-accept-gesture` |
     | [scroll-view](https://developers.weixin.qq.com/miniprogram/dev/api/wxml/NodesRef.scrollOffset.html) | `bind:scroll-start` | `worklet:onscrollstart` |
     | [scroll-view](https://developers.weixin.qq.com/miniprogram/dev/api/wxml/NodesRef.scrollOffset.html) | `bind:scroll` | `worklet:onscrollupdate` |
     | [scroll-view](https://developers.weixin.qq.com/miniprogram/dev/api/wxml/NodesRef.scrollOffset.html) | `bind:scroll-end` | `worklet:onscrollend` |
     | [scroll-view](https://developers.weixin.qq.com/miniprogram/dev/api/wxml/NodesRef.scrollOffset.html) | `adjust-deceleration-velocity` | `worklet:adjust-deceleration-velocity` |
...
5. `2023-07-28` 支持兼容模式下 WXS 事件响应中 `ComponentDescriptor` 的 `getState` 方法
   - 需要基础库版本 ['3.0.0'](../../compatibility) 或以上，正在逐步支持到版本 ['2.19.2'](../../compatibility)
6. `2024-05-20` 支持全空的数据绑定
   - 重新预览或上传即可，无版本依赖
7. `2024-10-18` 支持在组件 JS 的 options 中定义 `styleIsolation` 和 `addGlobalClass`
   - 需要基础库版本 ['3.6.3'](../../compatibility) 或以上，后续争取兼容到版本 ['3.0.0'](../../compatibility)
8. `2024-10-28` 支持 WXS 事件响应函数
   - 需要基础库版本 ['3.6.4'](../../compatibility) 或以上，后续争取兼容到版本 ['3.0.0'](../../compatibility)
