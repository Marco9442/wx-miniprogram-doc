...
对于具体样式兼容的策略上，由于 Skyline 中部分样式的默认值与 web 不同，因使用默认值而省略的样式需要显示指定，如 `flex-direction: row`，但此处更推荐[开启默认 Block 布局](../wxss#开启默认Block布局)和[默认 ContentBox 盒模型](../wxss#开启默认contentbox盒模型)，默认值处理与 web 更接近，其它更多信息，详见 [Skyline WXSS 样式支持与差异](../wxss)

### [#](#根据不同-renderer-兼容) 根据不同 renderer 兼容
...
- **weui 扩展库无法使用**
  ...
  平台正在支持扩展库，预计近期上线。建议开发者使用 npm 安装 [weui 组件库](https://github.com/wechat-miniprogram/weui-miniprogram) 后，将 node\_modules/weui-miniprogram 下的 miniprogram\_dist 替换为 [链接](https://github.com/wechat-miniprogram/weui-miniprogram/blob/feat-skyline-524/miniprogram_dist.zip) 中的 miniprogram\_dist，然后在微信开发者工具中构建 npm 即可。
- **不支持组件 animate 动画接口**
...
- **自定义组件的样式表现不正确**
  ...
  - 可留意是否存在跨自定义组件的样式匹配，Skyline 下 tag 和 id 选择器不支持跨自定义组件匹配，而 class 则遵循 [组件样式隔离机制](../../../custom-component/wxml-wxss#组件样式隔离)，可[开启 tag 选择器全局匹配](../wxss#开启tag选择器全局匹配)以保持与旧版行为对齐
