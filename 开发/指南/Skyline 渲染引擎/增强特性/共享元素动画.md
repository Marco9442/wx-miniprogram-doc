...
![](https://res8.wxqcloud.qq.com.cn/wxdoc/694b03ae-1799-4dbc-9bd5-beac6593908e.png)
...
![](https://res8.wxqcloud.qq.com.cn/wxdoc/1ba61667-ae3b-4dbc-9a69-c891c2e01365.png)
...
![](https://res8.wxqcloud.qq.com.cn/wxdoc/116cf0c7-d1d1-4f59-9ebc-0843e4f9f7b3.png)
...
![](https://res8.wxqcloud.qq.com.cn/wxdoc/69b146a9-d2dc-47a0-9f77-13b727f3108a.png)

### [#](#动画过程中) 动画过程中

根据指定的插值方式和动画曲线，目标 `share-element` 在 `overlay` 层进行动画，从起点向终点位置过渡。

![](https://res8.wxqcloud.qq.com.cn/wxdoc/c7c6b61d-32b0-4356-8789-b09381951d3f.png)

### [#](#动画结束时刻) 动画结束时刻

`progress = 1` 时刻，将目标 `share-element` 从 `overlay` 层移动到目标页面，出现在终点位置。

![](https://res8.wxqcloud.qq.com.cn/wxdoc/f00a3636-2b8d-4836-bb92-9f506fcb8bd1.png)
...
### [#](#Q1-设置了相同的-key，但没有看到飞跃动画) Q1: 设置了相同的 key，但没有看到飞跃动画

共享元素动画需保证下一个页面首帧即创建好 `share-element` 节点，并设置了 key，用于计算目标位置。

如果是通过 `setData` 设置的，可能会错过首帧。针对这种情况，可以 [使用 Component 构造器构造下一个页面](../../custom-component/component)，只要在组件 `attached` 生命周期前（含）通过 `setData` 设置上去，就会在首帧渲染。

### [#](#Q2-飞跃过程中，子节点不会跟随放大-缩小) Q2: 飞跃过程中，子节点不会跟随放大/缩小
...
![](https://res8.wxqcloud.qq.com.cn/wxdoc/ca8ecf0d-c436-4943-a9b9-d2e511085f9c.gif) ![](https://res8.wxqcloud.qq.com.cn/wxdoc/b47944fd-2c4e-4527-9ea8-c0ac966186b1.gif)
...
**默认计算方式，以 `share-element` 节点占据空间确定始末位置，会产生跳变。** ![](https://res8.wxqcloud.qq.com.cn/wxdoc/e0d886cd-6045-485d-8907-7615904aaa47.png)

**以图片实际占据空间确定始末位置，始终采用单一 `mode` 效果，不会产生跳变。** ![](https://res8.wxqcloud.qq.com.cn/wxdoc/17940ff8-db93-4c3f-9a0e-0dbc4e1d29f2.png)

使用最新 `nightly` 工具预览，移动端安卓 8.0.33 版本。

[示例代码片段-进阶](https://developers.weixin.qq.com/s/vZ8Ydymg7ZMs)

![](https://res.wx.qq.com/wxdoc/dist/assets/img/hero-animated-image.83787fa4.gif)
