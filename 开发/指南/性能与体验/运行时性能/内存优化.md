...
## [#](#_2-使用按需注入和用时注入) 2. 使用[按需注入](../../ability/lazyload#按需注入)和[用时注入](../../ability/lazyload#用时注入)
...
## [#](#_5-小程序常见的内存泄露问题) 5. 小程序常见的内存泄露问题

存在内存泄露问题会导致小程序在运行过程中内存占用持续增长，引起小程序闪退或被微信强制销毁。

### [#](#_5-1-小程序长期持有页面实例，导致页面实例和引用的组件无法正常销毁) 5.1 小程序长期持有页面实例，导致页面实例和引用的组件无法正常销毁
