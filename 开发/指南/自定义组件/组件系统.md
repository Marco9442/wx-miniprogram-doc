...
**提示:** 你可以通过 [组件样式隔离](wxml-wxss#组件样式隔离) 或 [外部样式类](wxml-wxss#外部样式类) 等方式，灵活地控制样式的隔离。

#### [#](#_2-事件隔离) 2. 事件隔离

通过 [triggerEvent](events#触发事件) 触发的事件会被限制在 Shadow 树内进行捕获和冒泡，外部组件无法直接监听这些事件。

**提示:** 你可以通过设置 `composed` 参数，改变事件的冒泡和捕获行为，从而允许外部监听。

#### [#](#_3-SelectQuery-隔离) 3. SelectQuery 隔离

使用 [SelectorQuery](https://developers.weixin.qq.com/miniprogram/dev/api/media/map/wx.createMapContext.html) 获取节点时，只能获取当前 Shadow 树内的节点，无法访问其他组件的 Shadow 树或外部元素。

#### [#](#_4-slot-状态) 4. slot 状态
...
**提示:** 你可以设置 [动态 slot](glass-easel/dynamic-slots) 改变此行为。

## [#](#Composed-树) Composed 树
...
![alt text](https://res8.wxqcloud.qq.com.cn/wxdoc/87a7ef5c-84e0-4294-af7e-caba71cbc2c9.png)
