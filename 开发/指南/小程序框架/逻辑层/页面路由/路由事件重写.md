...
从基础库 ['3.8.0'](../compatibility) 起，小程序可以在路由事件下发到基础库但还未进行实际处理之前，改变这次路由事件的目标页面路径及参数。这有一点类似 HTTP 协议中 URL 重定向的效果，但为了不与现有的 *页面重定向* `redirectTo` 混淆，我们将这种新的特性称为 **路由重写（Route rewrite）**。
