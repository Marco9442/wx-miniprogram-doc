...
页面导航条配置节点，用于指定导航栏的一些属性。只能是 [page-meta](page-meta.html) 组件内的第一个节点，需要配合它一同使用。

通过这个节点可以获得类似于调用 [`wx.setNavigationBarTitle`](../api/ui/navigation-bar/wx.setNavigationBarTitle.html) [`wx.setNavigationBarColor`](../api/ui/navigation-bar/wx.setNavigationBarColor.html) 等接口调用的效果。
