...
页面属性配置节点，用于指定页面的一些属性、监听页面事件。只能是页面内的第一个节点。可以配合 [navigation-bar](navigation-bar.html) 组件一同使用。

通过这个节点可以获得类似于调用 [`wx.setBackgroundTextStyle`](../api/ui/background/wx.setBackgroundTextStyle.html) [`wx.setBackgroundColor`](../api/ui/background/wx.setBackgroundColor.html) 等接口调用的效果。
...
[在开发者工具中预览效果](https://developers.weixin.qq.com/s/C0aAxomq7s9n "在开发者工具中预览效果")
