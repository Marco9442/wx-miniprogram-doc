...
> 相关文档: [画布指南](../framework/ability/canvas.html)、[Canvas 接口](../api/canvas/Canvas.html)、[旧版画布迁移指南](../framework/ability/canvas-legacy-migration.html)

渲染框架支持情况：Skyline （使用最新 [Nightly](/miniprogram/dev/devtools/nightly.html) 工具调试）、WebView

## [#](#功能描述) 功能描述

画布。2.9.0 起支持一套新 Canvas 2D 接口（需指定 type 属性），同时支持[同层渲染](native-component.html#%E5%8E%9F%E7%94%9F%E7%BB%84%E4%BB%B6%E5%90%8C%E5%B1%82%E6%B8%B2%E6%9F%93)，原有接口不再维护。旧版本可参考 [旧版画布迁移指南](../framework/ability/canvas-legacy-migration.html) 进行迁移。
...
3. `tip`：请注意[原生组件使用限制](./native-component.html#原生组件的使用限制)
...
10. `tip`: 在 mac 或 windows 小程序下，若当前组件所在的页面或全局开启了 `enablePassiveEvent` 配置项，该内置组件可能会出现非预期表现（详情参考 [enablePassiveEvent 文档](../reference/configuration/app)）
11. `tip`: 鸿蒙 OS 下暂不支持外接纹理

## [#](#Canvas-2D-示例代码) Canvas 2D 示例代码
...
[在开发者工具中预览效果](https://developers.weixin.qq.com/s/oyVplmmG6xYt "在开发者工具中预览效果") [下载](require('../demo/api-canvas.zip'))
