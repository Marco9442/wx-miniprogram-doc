...
> 相关文档: [wx.createMapContext](../api/media/map/wx.createMapContext.html)

渲染框架支持情况：Skyline （使用最新 [Nightly](/miniprogram/dev/devtools/nightly.html) 工具调试）、WebView

## [#](#功能描述) 功能描述

地图 v2.7.0 起支持[同层渲染](native-component.html#%E5%8E%9F%E7%94%9F%E7%BB%84%E4%BB%B6%E5%90%8C%E5%B1%82%E6%B8%B2%E6%9F%93)。
...
1. 自2023年6月29日0点起，该能力需要先购买再使用。若未购买，届时将无法使用该能力。具体购买方式见 [付费管理](../framework/open-ability/laboruse/intro.html)。
2. 自2023年6月29日0时起，个性化地图配置界面的入口统一为[微信公众平台-付费管理](../framework/open-ability/laboruse/intro.html)，请从此入口进入，腾讯位置服务官网入口不再使用。已经在小程序生效的个性化样式配置，将于2023年6月29日0时变更为默认样式，如有个性化样式配置需求，请于6月29日0时前，前往[微信公众平台-付费管理](../framework/open-ability/laboruse/intro.html)进行相关能力的开通和配置。
...
## [#](#regionchange-返回值) regionchange 返回值

视野改变时，regionchange 会触发两次，返回的 type 值分别为 begin 和 end。

[2.8.0](../framework/compatibility.html) 起 begin 阶段返回 causedBy，有效值为 gesture（手势触发） & update（接口触发）

[2.3.0](../framework/compatibility.html) 起 end 阶段返回 causedBy，有效值为 drag（拖动导致）、scale（缩放导致）、update（调用更新接口导致）。
...
## [#](#marker-碰撞关系) marker 碰撞关系

在一定范围内绘制多个 Marker 时，常会出现 Marker 压盖的情况，[3.4.3](../framework/compatibility.html) 起支持设置碰撞关系。
...
## [#](#marker-上的气泡-label) marker 上的气泡 label
...
1. [MapContext.initMarkerCluster](../api/media/map/MapContext.initMarkerCluster.html) 对聚合点进行初始化配置（可选）；
2. [MapContext.addMarkers](../api/media/map/MapContext.addMarkers.html) 指定参与聚合的 marker；
3. `MapContext.on('markerClusterCreate', callback)` 触发时，通过 [MapContext.addMarkers](../api/media/map/MapContext.addMarkers.html) 更新聚合簇的样式 （可选）；
4. [MapContext.removeMarkers](../api/media/map/MapContext.removeMarkers.html) 移除参与聚合的 marker；
...
2. 自定义聚合簇样式时，同样通过 [MapContext.addMarkers](../api/media/map/MapContext.addMarkers.html) 进行绘制，此时需携带 clusterId。
...
## [#](#bindregionchange-返回值) bindregionchange 返回值
...
## [#](#bindabilitysuccess、bindabilityfail-和-binderror-的返回值) bindabilitysuccess、bindabilityfail 和 binderror 的返回值
...
4. `tip`: `map` 组件使用的经纬度是火星坐标系，调用 [wx.getLocation](../api/location/wx.getLocation.html) 接口需要指定 `type` 为 `gcj02`
5. `tip`：从 2.8.0 起 map 支持同层渲染，更多请参考[原生组件使用限制](native-component.html#%E5%8E%9F%E7%94%9F%E7%BB%84%E4%BB%B6%E7%9A%84%E4%BD%BF%E7%94%A8%E9%99%90%E5%88%B6)
6. `tip`：请注意[原生组件使用限制](./native-component.html#原生组件的使用限制)。
7. `tip`: 若当前组件所在的页面或全局开启了 `enablePassiveEvent` 配置项，该内置组件可能会出现非预期表现（详情参考 [enablePassiveEvent 文档](../reference/configuration/app)）
