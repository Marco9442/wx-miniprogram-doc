...
可移动的视图容器，在页面中可以拖拽滑动。[movable-view](movable-view.html)必须在 [movable-area](movable-area.html) 组件中，并且必须是直接子节点，否则不能移动。
...
## [#](#bindchange-返回的-source-表示产生移动的原因) `bindchange` 返回的 `source` 表示产生移动的原因
