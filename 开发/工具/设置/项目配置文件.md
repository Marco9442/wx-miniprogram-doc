...
| [staticServerOptions](https://developers.weixin.qq.com/minigame/dev/devtools/projectconfig#staticServerOptions) | Object | 仅在小游戏项目中有效 |
| editorSetting | Object | 指定自动生成的文件的 tabIndent 和 tabSize |
| [skeletonConfig](skeleton) | Object | 骨架屏相关设置 |

### [#](#compileType) compileType

编译类型作用于开发工具的以下设置
![编译类型](https://res8.wxqcloud.qq.com.cn/wxdoc/aa935d99-3bf5-48e8-876c-747b980adc0d.png)
有效值
...
![详情-本地设置](https://res8.wxqcloud.qq.com.cn/wxdoc/9620fa83-1512-4fc9-8be1-b0470d90d8bc.png)

| 字段名 | 类型 | 允许私有设置[1] | 说明 |
| --- | --- | --- | --- |
| condition | Boolean | 否 | 启用[条件编译](https://developers.weixin.qq.com/miniprogram/dev/platform-capabilities/miniapp/pre-read/condition-compile) |
...
| packNpmRelationList | Array | 否 | 仅 `packNpmManually` 为 true 时生效，详细参考[构建 npm 文档](npm) |
...
**注 3**: (**懒注入占位组件调试**) 开启[按需注入](https://developers.weixin.qq.com/miniprogram/dev/framework/ability/lazyload)后在项目设置面板中会出现对应的勾选项，开启之后，按需注入的组件将会停止注入，页面停止在占位组件状态，便于调试自定义占位组件。
...
#### [#](#minifyWXMLSetting) minifyWXMLSetting
...
目前可以指定 `packOptions.include` 字段，用以配置打包时需要强制带上的文件（[仅限后缀名白名单内](https://developers.weixin.qq.com/miniprogram/dev/framework/structure#允许上传的文件)）或者文件夹，匹配的这些文件或文件夹将一定会出现在预览或上传的结果内。（该字段的优先级高于 `packOptions.ignore`）
