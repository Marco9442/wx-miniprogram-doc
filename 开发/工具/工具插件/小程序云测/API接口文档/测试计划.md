...
注意： 通过第三方接口创建、修改测试计划只支持[自定义测试](minium) 任务，其他类型请在云测页面进行配置。
...
| test\_plan\_name | 是 | string | 测试计划名称 |
| test\_plan\_config | 是 | string | 参照 [查看编写说明](https://minitest.weixin.qq.com/#/minium/Python/framework/suite)，注意这个字段需要Json序列化成String传入(使用json.dumps) |
...
| group\_en\_id | 是 | string | 项目英文ID，可在 项目管理/产品管理 页面获取 |
| test\_plan\_name | 是 | string | 测试计划名称 |
| case\_ids | 是 | string | AI用例ID，需要用json.dumps转成字符串，例如 `json.dumps([1])` |
...
| token | 是 | string | 可以从页面右上角头像右侧下拉菜单中的 “我的Token” 获取 |
| group\_en\_id | 是 | string | 项目英文ID，可在 项目管理/产品管理 页面获取 |
| test\_type | 是 | int | 测试类型，`1`: Monkey`2`：Minium `3`：录制回放 |
| page\_index | 否 | int | 第 n 页，从1开始，默认-1，表示不分页 |
| page\_size | 否 | int | 每页返回数量，默认为-1，表示不分页 |
| keyword\_in\_name | 否 | string | 筛选条件，测试计划名称包含的字符 |
...
| token | 是 | string | 可以从页面右上角头像右侧下拉菜单中的 “我的Token” 获取 |
| group\_en\_id | 是 | string | 项目英文ID，可在 项目管理/产品管理 页面获取 |
| test\_plan\_id | 是 | int | 测试计划id，可通过查询测试计划api获取或测试计划页面id列获取 |
| test\_plan\_name | 是 | string | 测试计划名称 |
...
| token | 是 | string | 可以从页面右上角头像右侧下拉菜单中的 “我的Token” 获取 |
| group\_en\_id | 是 | string | 项目英文ID，可在 项目管理/产品管理 页面获取 |
| test\_plan\_id | 否 | int | 测试计划id，可通过查询测试计划api获取或测试计划页面id列获取 |
| test\_plan\_name | 否 | string | 测试计划名称 |
| priority | 否 | string | 优先级，只能在'P0', 'P1', 'P2', 'P3', 'P4', 'P5', 'Others'选择 |
| case\_ids | 否 | string | AI用例ID，需要用json.dumps转成字符串，例如 `json.dumps([1])` |
