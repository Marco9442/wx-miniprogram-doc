...
| --- | --- | --- |
| 帮助 | 查看帮助 | cli -h |
| 登录 | [登录工具](#登录工具) | cli login |
|  | [是否登录工具](#是否已经登录工具) | cli islogin |
| 小程序代码 | [预览](#预览) | cli preview |
|  | [上传代码](#上传代码) | cli upload |
|  | [自动预览](#自动预览) | cli auto-preview |
|  | [开启自动化](#开启自动化) | cli auto |
|  | [开启自动化](#打开自动化测试窗口) | cli auto-replay |
|  | [构建 npm](#构建-npm) | cli build-npm |
|  | [清除缓存](#清除工具缓存) | cli cache |
| 工具窗口 | [启动工具](#启动工具) | cli open |
|  | [打开其他项目](#打开其他项目) | cli open-other |
|  | [关闭项目窗口](#关闭项目窗口) | cli close |
|  | [关闭工具](#关闭工具) | cli quit |
|  | [重建文件监听](#重建文件监听) | cli reset-fileutils |
| 云开发 | [云开发操作](#云开发操作) | cli cloud -h |
|  | [云环境相关操作](#云开发操作) | cli cloud env -h |
|  | [云函数相关操作](#云开发操作) | cli cloud functions -h |
|  | [查看云环境列表](#查看云环境列表) | cli cloud env list |
|  | [查看云函数列表](#查看云函数列表) | cli cloud functions list |
|  | [查看云函数信息](#查看云函数信息) | cli cloud functions info |
|  | [上传云函数](#上传云函数) | cli cloud functions deploy |
|  | [增量上传云函数](#增量上传云函数) | cli cloud functions inc-deploy |
|  | [下载云函数](#下载云函数) | cli cloud functions download |
...
**`--version, -v`**: 上传代码，version 指定版本号，project\_root 指定项目根路径。
...
**`--project`**: 可选，如果指定的 project\_root 项目被打开，将会被自动关闭
...
**`--names, -n`**: 云函数名称，多个云函数则以空格分隔，例：cli cloud functions info --names func\_a func\_b
...
**`--remote-npm-install, -r`**: 云端安装依赖，指定选项后 node\_modules 将不会上传
