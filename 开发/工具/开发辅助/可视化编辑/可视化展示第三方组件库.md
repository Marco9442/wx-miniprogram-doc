## [#](#可视化编辑支持第三方组件库展示能力) 可视化编辑支持第三方组件库展示能力

> 对于已有的小程序组件库，工具可视化新增展示能力。只需该组件库包处于 miniprogram\_npm 目录下，且在组件库根目录下配置相应的配置文件接口
> 工具可视化即可解析和添加该组件库组件信息，使其可以拖拽到相应页面 wxml 和 json 中。

## [#](#运行环境) 运行环境

[1.05.2206242](log#rc) 或以上版本的开发者工具
...
- 在组件库的根目录中新建一个 `.wechatide.ib.json`，需要打包后的 miniprogram\_root

## [#](#_2-配置组成说明) 2. 配置组成说明

### [#](#_2-1-主要结构-components-和-menu) 2.1 主要结构 components 和 menu

2. `.wechatide-ib.json` 内部是一个 json 对象，包含以下 `key`
...
### [#](#_2-2-组件定义-IIBComponentLibConfigComponent) 2.2 组件定义 IIBComponentLibConfigComponent
...
#### [#](#_2-2-1-组件自定义模板-tpl) 2.2.1 组件自定义模板 tpl
...
#### [#](#_2-2-2-组件依赖路径-path) 2.2.2 组件依赖路径 path
...
#### [#](#_2-2-3-组合组件依赖说明-require) 2.2.3 组合组件依赖说明 require
...
#### [#](#_2-2-4-组件事件-IComponentEvent) 2.2.4 组件事件 IComponentEvent
...
#### [#](#_2-2-5-组件属性-IComponentProperty) 2.2.5 组件属性 IComponentProperty
...
#### [#](#_2-2-6-复用公共的-properties-和-event-暂不支持) 2.2.6 复用公共的 properties 和 event(暂不支持)
