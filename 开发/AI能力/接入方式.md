    ...
    "skills": [
      {
        "name": "weatherSkill",
        "description": "查询天气业务",
        "path": "path/to/pkg/weatherSkill"
      },
      {
        "name": "shoppingSkill",
        "description": "购物业务",
        "path": "path/to/pkg/shoppingSkill"
      }
    ]
  }
}
...
### [#](#_3-1-模型可调用能力的声明) 3.1 模型可调用能力的声明
...
}) => { query: string; payload: Record<string, unknown> }
```

在原子接口的内部实现中，可以通过 `wx.request` 从服务器请求数据，也可以通过[微信云开发](https://developers.weixin.qq.com/miniprogram/dev/wxcloudservice/wxcloud/basis/getting-started)调用云函数或数据库获取数据。
...
async function getWeather({ location, days }) {
  // 原子接口内部实现，此处省略
  // wx.request({ url: 'https://www.example.com/get_weather' })
  // wx.cloud.callFunction({ name: 'get_weather' })
...
#### [#](#显式指定下一步调用的原子接口) 显式指定下一步调用的原子接口

在某些较固定流程的场景中，需要在一次原子接口调用之后，明确要触发下一个原子接口的调用。此时，可显式指定要调用的原子接口，以减少模型推理的耗时。

```
wx.modelContext.registerAPI('placeOrder', async ({ skuId, count }) => {
  ...
  return {
    isError: false,
    content: [{ type: 'text', text: `订单已创建` }],
  }
})
```

### [#](#_3-3-注册原子接口) 3.3 注册原子接口
...
skill.use(async (ctx, next) => {
  ...
  if (!token) {
    ...
    // 也可以直接抛出错误，让Agent进行其他操作
    // throw new Error('登录过期，接下来自动执行xxx skill 下的 login 工具')
  }
  // 2. 上报：记录开始时间
...
#### [#](#_1、在-mcp-json-声明原子接口对应的-pagePath) 1、在 mcp.json 声明原子接口对应的 pagePath
...
#### [#](#_2、原子接口返回-handoff-对象) 2、原子接口返回 handoff 对象
...
async function getWeather({ location, days }) {
  ...
  return {
    ...
    structuredContent: {},
    handoff: (data) => {
      // data.result.structuredContent 模型根据用户意图过滤后数据
      return {
        query: 'poi=beijing&days=3',
        payload: {
          location: '北京',
          forecasts: [
            { date: "2026-04-23", dayWeather: "晴", nightWeather: "晴", tempHigh: 28, tempLow: 14, wind: "北风2级" },
...
#### [#](#_3、小程序监听-wx-onAgentHandoff-获取-handoff-数据) 3、小程序监听 wx.onAgentHandoff 获取 handoff 数据
...
```
{
  pageId: number    // 目标页实例 ID，用于关联跳转的页面，精确投递 payload
  path: string      // 当前打开的页面的 path（不含 query）
  query: string     // 页面 query 字符串，与原子接口返回的 handoff.query 相同
  payload: unknown  // 原子接口返回的 handoff.payload
}
```
...
Page({
  onLoad(query) {
    ...
    // 使用原子接口返回的数据更新页面状态
    this.setData({ forecasts: handoff.payload.forecasts })

    // 使用 query 参数拉取数据
    this.loadPoiDetail(query.poi, query.days).then(forecasts => this.setData({ forecasts }))
  },
})
```

总体上，可参考[示例 demo](https://github.com/wechat-miniprogram/ai-mode-demo)

### [#](#_3-5-原子组件实现) 3.5 原子组件实现

> 暂不支持调试

#### [#](#_3-5-1-原子组件定义) 3.5.1 原子组件定义
...
#### [#](#_3-5-2-关联小程序页面（必须）) 3.5.2 关联小程序页面（必须）

原子组件渲染出来的 GUI 卡片上方有个标题栏，其右上方提供进入小程序的入口，该入口需配置与原子组件关联的小程序页面，是必填项。通过此方式进入小程序的场景值为 1442 或 1443。

![](https://res8.wxqcloud.qq.com.cn/wxdoc/c8904191-99cc-4142-8b16-d11d138f475e.png)
...
#### [#](#_3-5-5-原子组件过期态) 3.5.5 原子组件过期态

支持通过接口形式，将已经渲染出来的原子组件置为过期态，防止用户再次点击。

![](https://res8.wxqcloud.qq.com.cn/wxdoc/f89e7541-d052-4792-af38-2fdb128e568f.png)
...
wx.modelContext.expireAllCards({
  componentPaths: ['packageA/weatherSkill/components/weather-card/index']
})
...
#### [#](#_3-5-7-同步交互状态给模型) 3.5.7 同步交互状态给模型
...
Component({
  methods: {
    onOptionSelect(e) {
      ...
      // 将用户选择同步给模型
      const viewCtx = wx.modelContext.getViewContext(this)
      viewCtx.updateModelContext({
        content: [
          { type: 'text', text: `用户选择了${option}` }
        ]
      })
    }
  }
...
#### [#](#_3-6-4-从半屏页面返回并更新卡片) 3.6.4 从半屏页面返回并更新卡片
...
![](https://res8.wxqcloud.qq.com.cn/wxdoc/940cd230-8333-4b39-8afc-a21c1a81d9d1.png)

**1. 上传文件**

上传入口： [微信公众平台](https://mp.weixin.qq.com/) > 基础功能 > AI能力 > 知识库
...
- **公众平台调试器**（简易效果）
  ...
  - 上传后快速验证文件是否被正确切片召回
  - 非C端最终回答效果，无上下文，无法调用`SKILL`

![](https://res8.wxqcloud.qq.com.cn/wxdoc/be5545bc-4d73-45c5-9a01-adf2ff93742e.png)
...
该能力需要开发者提供页面元数据，小程序 AI 将根据用户问答的上下文，在满足用户意图时，生成小程序账号卡片发送给用户。通过此方式进入小程序的场景值为 1435 或1436。
