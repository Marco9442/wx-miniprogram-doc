...
最简单的情况，「小程序」对「直属微信云托管环境」的服务进行访问，也就是登录[微信云托管控制台](https://cloud.weixin.qq.com/cloudrun?utm_source=wxrundoc&utm_content=dev_mpcall)时选择小程序进入的环境。

以上情况我们约定为`A小程序`，如果你希望另外一个 `B小程序`/`B公众号` 也可以访问 `A小程序` 的云托管环境：

1. `A小程序` 和 `B小程序`/`B公众号`同主体：需要配置[资源复用](../../guide/reuse/index)。
2. `A小程序` 和 `B小程序`/`B公众号`不同主体：推荐使用「[微信开放平台-第三方平台](https://developers.weixin.qq.com/doc/oplatform/Third-party_Platforms/2.0/product/Third_party_platform_appid)」方式。
3. `A小程序`和 `B小程序`/`B公众号`不同主体：将`B小程序`/`B公众号`视为「其他客户端」，通过公网访问。参考文档[其他客户端-访问云托管服务](client)。此方式下，`B小程序`/`B公众号`无法使用云调用/微信令牌，且需要配置「服务器域名」（使用云托管服务的默认公网域名/自定义域名均可）。

> 如果你对以上正在支持的能力有任何建议和期待，欢迎在[官方交流群](https://www.cloudbase.vip/scan/?code=werun)联系我们。
...
`wx.cloud.callContainer` 其他参数，直接参考 [wx.request](https://developers.weixin.qq.com/miniprogram/dev/api/network/request/wx.request.html) API，在这里列举常用参数：
...
小程序向云托管服务发起callcontainer调用时，你的服务请求header中会自动带有[用户信息](../weixin/index)，包括openid、unionid、ip地址、可信来源等等，无需再通过小程序wx.login登录，然后调接口置换，大幅简化了流程。

> 注意，资源复用情况下，获取openid的字段和普通获取不一致。

## [#](#使用限制) 使用限制

1. 请求大小限制100KiB (对象类型限制 20 MiB，请求中不建议包含图片，可通过对象存储处理)；
2. 返回包大小限制 1000KiB。
