# [#](#快速入门-Python自定义部署) 快速入门-Python自定义部署
...
### [#](#_3-创建app目录，并在目录中创建main-py-文件，并在文件中填入如下代码) 3. 创建`app`目录，并在目录中创建`main.py` 文件，并在文件中填入如下代码
...
在第二步中，访问服务获取的默认域名地址，就是服务对外提供服务的入口，你可以将其按照正常的服务来调用访问，比如上述Python例子中，就可以用以下来调用：
...
以上Python例子访问的代码如下（在小程序项目 `app.js` 中覆盖写入如下代码）
...
以上Python例子在浏览器中访问的代码如下（请将以下代码放置在html文件中，并在浏览器里运行控制台看结果）
