...
从小程序端调用云函数时，开发者可以在云函数内使用 [`wx-server-sdk`](wx-server-sdk) 提供的 [`getWXContext`](../../reference-sdk-api/utils/Cloud.getWXContext) 方法获取到每次调用的上下文（`appid`、`openid` 等），无需维护复杂的鉴权机制，即可获取天然可信任的用户登录态（`openid`）。可以写这么一个云函数进行测试：
...
在[下一章节](async)，我们一起看看如果在云函数中需要进行一段异步操作再返回的时候该如何处理。
