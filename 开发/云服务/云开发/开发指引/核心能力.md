...
在小程序端开始使用云能力前，需先调用 `wx.cloud.init` 方法完成云能力初始化（注意小程序需先开通云服务，开通的方法是点击工具栏左上角的 “云开发” 按钮）。

因此，如果要使用云能力，通常我们在小程序初始化时即调用这个方法。
...
| 字段 | 数据类型 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
| env | string | object | 是 |  | 后续 API 调用的默认环境配置，传入字符串形式的环境 ID 或传入 [`cloud.DYNAMIC_CURRENT_ENV`](../reference-sdk-api/constant/constant) 可以指定所有服务的默认环境，传入对象可以分别指定各个服务的默认环境，见下方详细定义 |
...
**建议**：在设置 `env` 时指定 [`cloud.DYNAMIC_CURRENT_ENV`](../reference-sdk-api/constant/constant) 常量 (需 SDK `v1.1.0` 或以上) ，这样云函数内发起数据库请求、存储请求或调用其他云函数的时候，默认请求的云环境就是云函数当前所在的环境：
