...
常量可以是任意类型。默认情况下 `$` 开头的字符串都会被当做字段路径处理，如果想要避免这种行为，使用 [`AggregateCommand.literal`](../command/aggregate/AggregateCommand.literal) 声明为常量。

#### [#](#聚合操作符) 聚合操作符

[AggregateCommand](../command/aggregate/AggregateCommand)
