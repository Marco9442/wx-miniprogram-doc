...
### [#](#Aggregate-bucket-object-Object-Aggregate) <Aggregate.bucket>(object: Object): <Aggregate>

聚合阶段。将输入记录根据给定的条件和边界划分成不同的组，每组即一个 `bucket`。

### [#](#Aggregate-bucketAuto-object-Object-Aggregate) <Aggregate.bucketAuto>(object: Object): <Aggregate>

聚合阶段。将输入记录根据给定的条件划分成不同的组，每组即一个 `bucket`。与 [`bucket`](Aggregate.bucket) 的其中一个不同之处在于无需指定 `boundaries`，`bucketAuto` 会自动尝试将记录尽可能平均的分散到每组中。
...
### [#](#Aggregate-project-object-Object-Aggregate) <Aggregate.project>(object: Object): <Aggregate>
