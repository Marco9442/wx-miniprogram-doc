...
重置指定的登录态 session\_key。为了保持 session\_key 私密性，接口不明文传入 session\_key，而是通过校验登录态签名完成。
...
1. 重置用户session\_key后，原有的session\_key会失效。请注意接口调用时机，以免影响用户在小程序中的体验。
2. 重置后的session\_key会继承原有的session\_key的过期时间，重置操作不能为session\_key续期。
