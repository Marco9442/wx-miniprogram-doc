...
1. 与 [getAccessToken](api_getaccesstoken) 获取的调用凭证完全隔离，互不影响。
2. 该接口仅支持 `POST` 形式的调用。
...
### [#](#_5-1-不强制刷新获取Token（不传递force-refresh，默认值为false）) 5.1 不强制刷新获取Token（不传递force\_refresh，默认值为false）
...
### [#](#_5-2-不强制刷新获取Token（设置force-refresh为false）) 5.2 不强制刷新获取Token（设置force\_refresh为false）:
...
### [#](#_5-3-强制刷新模式，慎用，连续使用需要至少间隔30s) 5.3 强制刷新模式，慎用，连续使用需要至少间隔30s
