# [#](#RequestTask-wx-request-Object-object) [RequestTask](RequestTask.html) wx.request(Object object)
...
### [#](#RequestTask) [RequestTask](RequestTask.html)
...
## [#](#useHighPerformanceMode-高性能模式说明) useHighPerformanceMode 高性能模式说明
...
- 暂不支持 HttpDNS 能力。
- 开启 `enableProfile` 后，返回的 profile 字段部分信息缺失，会被缺省值代替。缺失部分包括 redirectStart、redirectEnd、rtt、estimate\_nettype、httpRttEstimate、transportRttEstimate、downstreamThroughputKbpsEstimate、throughputKbps、peerIP、port。
