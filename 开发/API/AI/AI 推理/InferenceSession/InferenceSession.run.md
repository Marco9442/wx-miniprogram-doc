# [#](#Promise-Tensors-InferenceSession-run-Tensors-tensors) Promise<[Tensors](Tensors.html)> InferenceSession.run([Tensors](Tensors.html) tensors)
...
### [#](#Tensors-tensors) [Tensors](Tensors.html) tensors

key-value 形式的对象，对象的 key 会作为 input name，对象的 value 则是 Tensor。 Tensor 结构如下。

## [#](#返回值) 返回值

### [#](#Promise-Tensors) Promise.<[Tensors](Tensors.html)>
